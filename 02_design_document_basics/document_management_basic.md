...
- 軽微な修正（誤字脱字・表記揺れ・リンク修正・意味を変えないレイアウト修正）は、本教材では **Version を据え置き、Change History または Git 履歴で管理する**。初学者が修正のたびに版を上げて混乱しないためで、本教材の各文書も意味のある変更のときだけ版を上げている。運用ルールはプロジェクトごとに統一してよい。
...
- 内容が承認され正式版になる場合は、プロジェクトのルールに従って 1.0 とする。何をもって 1.0 とするかは本教材では断定せず、プロジェクトごとに定める。
