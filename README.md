...
> 本READMEは設計書群ではなく教材集の入口であるため、solacom_main の設計標準README（`docs/standards/templates/design/README.md`）の管理形式を継承しつつ、画面概要（Screen Overview）・設計書一覧（Design Documents Index）を、教材構成概要（Learning Materials Overview）・教材一覧（Learning Materials Index）へ置換している。

---

## Table of Contents（目次）

1. [Project Overview（プロジェクト・教材概要）](#1-project-overviewプロジェクト教材概要)
2. [Problem / Solution / Benefit Summary（課題・解決・効果の概要）](#2-problem--solution--benefit-summary課題解決効果の概要)
3. [Learning Materials Overview（教材構成概要）](#3-learning-materials-overview教材構成概要)
4. [Learning Materials Index（教材一覧）](#4-learning-materials-index教材一覧)
5. [Overall Learning Policy（教材全体方針）](#5-overall-learning-policy教材全体方針)
...
## 1. Project Overview（プロジェクト・教材概要）

SCAO Learning Kit は、IT職業訓練校の生徒や初学者が、AIを活用してポートフォリオや小規模アプリを作成する際に、目的・要求・要件・設計・AIへの依頼内容を整理するための教材テンプレート集である。

本リポジトリは企業の本番開発手順を完全再現するものではないが、実務で重要となる設計・データ・セキュリティ・エラー処理・レビューの考え方を、ポートフォリオ作成に必要な範囲で取り入れる。必ずL1から順に読む「教科書」ではなく、目的別に参照する「教材集」である。

---

## 2. Problem / Solution / Benefit Summary（課題・解決・効果の概要）

各項目は概要のみを1〜2文で示す。詳細は各Level・各教材本文で扱う。

| Item（項目） | Summary（概要） | Detail Materials（詳細教材） |
|---|---|---|
...
| Development Purpose（開発目的） | AI活用とドキュメント整理の基礎を、初学者が短時間で身につけられる教材を提供する。 | [L1](./01_ai_basics/README.md) / [L2](./02_design_document_basics/README.md) |
| Solution Approach（解決方針） | AIへの頼み方・安全・文脈管理（L1）と、要求・要件・簡易設計・ドキュメント管理（L2）を、解説とコピー可能なテンプレートで学べるようにする。 | [L2 テンプレート](./02_design_document_basics/templates/README_TEMPLATE.md) |
| Learning Functions（教材機能） | 解説教材・移行テンプレート・4種の設計テンプレート・自己帰属バイアス（簡易版／詳細版）を、Level別に提供する。 | [L1](./01_ai_basics/README.md) / [L2](./02_design_document_basics/README.md) |
...
## 3. Learning Materials Overview（教材構成概要）

本教材集は、Level（L1〜L4）で構成する。L1・L2は初版として利用可能、L3・L4は準備中である。

- L1：AIを安全かつ正確に使うための基本（頼み方・Markdown・安全・LITM・自己帰属バイアス・移行テンプレート）。
- L2：作る前に整理すべき設計ドキュメントの基本（README・要求・要件・簡易設計・ドキュメント管理）と、コピーして使える4テンプレート。
...
## 4. Learning Materials Index（教材一覧）

| Level | Material（教材） | Status（状態） | Link（リンク） |
|---|---|---|---|
...
| L2 | README_TEMPLATE | Draft | [templates/README_TEMPLATE.md](./02_design_document_basics/templates/README_TEMPLATE.md) |
| L2 | REQUEST_TEMPLATE | Draft | [templates/REQUEST_TEMPLATE.md](./02_design_document_basics/templates/REQUEST_TEMPLATE.md) |
| L2 | REQUIREMENTS_TEMPLATE | Draft | [templates/REQUIREMENTS_TEMPLATE.md](./02_design_document_basics/templates/REQUIREMENTS_TEMPLATE.md) |
| L2 | SIMPLE_DESIGN_TEMPLATE | Draft | [templates/SIMPLE_DESIGN_TEMPLATE.md](./02_design_document_basics/templates/SIMPLE_DESIGN_TEMPLATE.md) |
...
## 5. Overall Learning Policy（教材全体方針）

- 本リポジトリは実務開発フレームワークそのものではなく、実務寄りの考え方を取り入れた学習用の教材集である。
- SCAO 本体（設計標準・フレームワーク群）とは別物であり、その導入を前提としない。SCAO 本体の考え方の一部を初学者向けに噛み砕いて取り入れているが、標準の全構造をそのまま適用するものではない。
- 対象読者は、IT職業訓練校の生徒、プログラミングやAI活用を学び始めた初学者、AIでポートフォリオや小規模アプリを作りたい人である。
- 秘密情報の取り扱い注意：教材の例やテンプレートに、実在するAPIキー・パスワード・トークン・秘密鍵・個人情報を記載しない。例示が必要な場合は `YOUR_API_KEY` のように、明らかにダミーと分かる文字列を使う。
...
|---|---|
| SCAO | 設計標準・フレームワーク群からなる本体。本教材集はその考え方の一部を学習用に噛み砕いたもので、本体とは別物。 |
...
| 簡易設計（Simple Design） | 要件を「どう作るか」に落とす、学習用・小規模向けの最小限の設計。 |
...
|---|---|---|
| Owner（管理者） | Takashi Oikawa | 全教材（All Materials） |
| Reviewer（レビュアー） | 未定（TBD） | 全教材（All Materials） |
